...
import { submitTransportForm, type FormState } from '@/lib/actions/form-actions'
...
export default function ModernTransportForm() {
  const t = useTranslations()
  const [state, formAction] = useActionState(submitTransportForm, initialState)
  ...
  useEffect(() => {
    if (!state.success && state.errors && Object.keys(state.errors).length > 0) {
      ...
      const fieldOrder = [
        ...
        'vpart', 'vpanzahl', 'wgw', 'wmasse', // Packaging and goods
